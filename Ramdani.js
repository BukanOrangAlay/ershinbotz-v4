			...
			query = args.join(" ")
              ...
              if (args.length < 1) return reply('Usernamenya?')
				...
				user = args.join(" ")
					...
					Ramdani.sendMessage(from, buff, image, {quoted: mek, caption: gstalk})
					break 
//------------------< Sewa >-------------------
       case 'sewa':
              if (!isGroup)return reply(mess.only.group)
...
break
        case 'pinterest':
                    ...
                    ini_buffer = await getBuffer(ini_url)
                    await Ramdani.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*Ini dia Ngab*' })
                    break      
        case 'shopee':
               try {
