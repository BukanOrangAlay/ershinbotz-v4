...
//update resepmasakan
//update semua fitur yang ada di rest api lolhuman di ganti ke rest api lain dengan fitur yang sama 
...
petik = '```'
fake = 'Copyrigth © 2022 P3MU14-UI All Reverse'
...
module.exports = Ramdani = async (Ramdani, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Ramdani.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "COPYRIGHT © 2022 P3MU14-UI . ALL REVERSE","listType": "SINGLE_SELECT","sections": list}}, {})
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/Ramdani3.jpg`)},"title": `COPYRIGHT © 2022 P3MU14-UI . ALL REVERSE`,"description": "Ramdani Official", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Ramdani Official","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
      ...
      mentions(cptl, x, true)
       ...
       case 'donasi':
        ...
        case 'creator':
               ...
               buttons = [{buttonId: '!website',buttonText:{displayText: 'WEBSITE'},type:1}]
...
❏ RUN IN : *『> HEROKU <』*
❏ THANKS TO  : *『> RAMDANI , LEXXY AND ALL CREATOR <』*
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
       ...
       case 'family100':
              ...
              anu = await axios.get(`http://zekais-api.herokuapp.com/family100?apikey=sKU9f2V4`)
       ...
       case 'tebaktebakan':
              ...
              get_result = await fetchJson(`https://api.xteam.xyz/game/tebaktebakan?APIKEY=${setting.xteamkey}`, {method: 'get'})
      ...
      case 'tebakjenaka':
       ...
       case 'caklontong':
              ...
              get_result = await fetchJson(`http://zekais-api.herokuapp.com/caklontong?apikey=sKU9f2V4`)
              get_result = get_result.result
              jawaban = get_result.jawaban
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
...
if (args.length < 1) return reply('mimpinya tentang apa ngab?')
 ...
 anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/artimimpi?q=${teks}&apikey=FuckBitch`, {method: 'get'})
				kata = anu.result
				reply(kata)
break
case 'pantun':
              ...
              if (args.length < 1) return reply('Judulnya?')
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=FuckBitch`, {method: 'get'})
					...
					Ramdani.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
              if (args.length < 1) return reply('Usernamenya?')
					...
					Ramdani.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
//------------------< Sewa >-------------------
       case 'sewa':
              if (!isGroup)return reply(mess.only.group)
...
case 'play2':   
                ...
                reply('Loading.... ')
				play = args.join(" ")
				anu = await fetchJson(`https://zenzapi.xyz/downloader/play/playmp4?query={$play}&apikey=069fe18dc7b8`)
...
*[Wait] Tunggu Sebentar..*`
				...
				Ramdani.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.url}.mp4`, quoted:freply, caption: 'Nih Gan'})
					break  
					case 'playy':   
				  if (args.length < 1) return reply('*Masukan judul nya ngab;?*')
                reply('Loading.... ')
				play = args.join(" ")
				anu = await fetchJson(`https://zenzapi.xyz/downloader/play/playmp3?query={$play}&apikey=069fe18dc7b8`)
				if (anu.error) return reply(anu.error)
				infomp3 = `*「 PLAY MP3 」*
...
if (args.length < 1) return reply('Judulnya ngab?')
				var teks = body.slice(14)
 anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/music/liriklagu?query={$args}&apikey=FuckBitch`, {method: 'get'})
				kata = anu.result
				reply(kata)
break
          case 'lirik2':
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              Ramdani.sendMessage('6289627655877@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
└⬣`)
      ...
      case 'qris':      
        qris =`*SCAN QRIS NYA OM, TEEF SEIHKLASNYA🗿*`
              ...
              Ramdani.relayWAMessage(prep)
                ...
                break
      case 'dana':
             reply(`DANA : 089627655877`)
             break
           
         case 'pulsa':
             reply(`*Pulsa :* 0895163677123`)
             break              
      case 'infoig':
             reply(`Follow Instagram Owner Ya : https://www.instagram.com/m1n1.php5`)
             break
     ...
     case 'toolsmenu':
              ...
              break                   
      case 'sholawatmenu':
             Ramdani.sendMessage(from, sholawatMenu(prefix), MessageType.text, {quoted: troli})
             break                                    
      case 'othermenu':
...
//--------------<soundnya vang:v>-----------------
      ...
      case 'sholawat':
             ...
             Ramdani.sendMessage(from, sholawat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
case 'sholawat1':
             sholawatsatu = fs.readFileSync('./media/sholawat1.mp3');
             Ramdani.sendMessage(from, sholawatsatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
case 'sholawat2':
             sholawatdua = fs.readFileSync('./media/sholawat2.mp3');
             Ramdani.sendMessage(from, sholawatdua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
case 'sholawat3':
             sholawattiga = fs.readFileSync('./media/sholawat.mp3');
             Ramdani.sendMessage(from, sholawattiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
case 'sholawat4':
             sholawatempat = fs.readFileSync('./media/sholawat.mp3');
             Ramdani.sendMessage(from, sholawatempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
